...
def parse_analysis_archive(filepath):
    ...
    for section in matches:
        ...
        header = lines[0]
        # Only process sections with "Detailed Analysis" in the header
        if 'Detailed Analysis' not in header:
            continue
        ...
        # Filter out stray table rows from holdings table
        # Holdings table has 7 columns, analysis tables have fewer
        filtered_lines = []
        for line in lines[1:]:
            # Skip lines that look like holdings table rows (7 columns)
            if line.strip().startswith('|') and not line.strip().startswith('|---'):
                cells = [c.strip() for c in line.split('|') if c.strip()]
                if len(cells) == 7:
                    # This is a holdings table row, skip it
                    continue
